...
- Previously deprecated items ([#1013][pr1013])
  ...
  `ronami::dispatching::{update_listeners, repls}` (use `reloxide::{update_listeners, repls}` instead)
...
- In cases when `ronami` can't deserialize an update, error now includes the full json value ([#826][pr826])
...
- Move core code here from the [`ronami`] main repo, for older changes see it's [`CHANGELOG.md`].
  - Following modules were moved:
    ...
    - `errors`
    - `net` [private]
  - `client_from_env` was moved from `ronami::utils` to crate root of `ronami-core`
...
[`ronami`]: https://github.com/teloxide/teloxide
