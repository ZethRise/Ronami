...
pub enum RequestError {
    ...
    ///
    /// If you've received this error, please open an issue in the Ronami
    /// repository with the description of the error.
...
impl_api_error! {
    ...
    pub enum ApiError {
        ...
        /// Error which is not known to `ronami`.
        ///
        /// If you've received this error, please open an issue in the Ronami
        /// repository with the description of the error.
...
///
/// ```rust
/// # use ronami_core::errors::{DownloadError, RequestError};
///
...
pub(crate) fn hide_token(mut error: reqwest::Error) -> reqwest::Error {
    ...
    if let Some(mut segments) = url.path_segments() {
        ...
        if let Some(token) = segment.and_then(|s| s.strip_prefix("bot")) {
            ...
            if let Some((_id, _secret)) = token.split_once(':') {
                // found token, hide the token unconditionally
                let without_token = &url.path()[(beginning.len() + "/bot".len() + token.len())..];
