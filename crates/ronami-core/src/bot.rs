...
const RONAMI_TOKEN: &str = "RONAMI_TOKEN";
const RONAMI_API_URL: &str = "RONAMI_API_URL";
...
/// ```
/// # async {
/// use ronami_core::prelude::*;
///
/// let bot = Bot::new("TOKEN");
/// dbg!(bot.get_me().await?);
/// # Ok::<_, ronami_core::RequestError>(()) };
/// ```
///
...
impl Bot {
    ...
    /// Creates a new `Bot` with the `RONAMI_TOKEN` & `RONAMI_API_URL` &
    /// `RONAMI_PROXY` environmental variables (the bot's token & the bot's
    /// API URL & the proxy) and the default [`reqwest::Client`].
    ///
    /// If `RONAMI_API_URL` doesn't exist, returns to the default TBA URL.
    ///
    /// This function passes the value of `RONAMI_PROXY` into
    /// [`reqwest::Proxy::all`], if it exists, otherwise returns the default
    /// client.
    ///
    /// # Panics
    ///  - If cannot get the `RONAMI_TOKEN`  environmental variable.
    ///  - If `RONAMI_API_URL` exists, but isn't a correct URL.
    ///  - If it cannot create [`reqwest::Client`].
    ///
    ...
    /// Creates a new `Bot` with the `RONAMI_TOKEN` environmental variable
    /// (the bot's token), `RONAMI_API_URL` environmental variable (the bot's
    ...
    pub fn from_env_with_client(client: Client) -> Self {
        let bot = Self::with_client(get_env(RONAMI_TOKEN), client);

        match std::env::var(RONAMI_API_URL) {
            Ok(env_api_url) => {
                ...
                let api_url = reqwest::Url::parse(env_api_url)
                    .expect("Failed to parse the `RONAMI_API_URL` env variable");
