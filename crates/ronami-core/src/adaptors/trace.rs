...
/// like these:
/// ```text
/// TRACE ronami_core::adaptors::trace > Sending `SendDice` request
/// TRACE ronami_core::adaptors::trace > Got response from `SendDice` request
/// TRACE ronami_core::adaptors::trace > Sending `SendDice` request: SendDice { chat_id: Id(0), emoji: Some(Dice), disable_notification: None, reply_to_message_id: None, allow_sending_without_reply: None, reply_markup: None }
/// TRACE ronami_core::adaptors::trace > Got response from `SendDice` request: Ok(Message { id: 13812, date: 1625926524, chat: Chat { .. }, via_bot: None, kind: Dice(MessageDice { dice: Dice { emoji: Dice, value: 3 } }) })
/// ```
#[derive(Clone, Debug)]
pub struct Trace<B> {
    inner: B,
...
bitflags::bitflags! {
    ...
    ///
    /// ```
    /// use ronami_core::adaptors::trace::Settings;
    ///
    /// // Trace nothing
