...
/// Constructs a network client from the `RONAMI_PROXY` environmental
/// variable.
///
...
/// ## Panics
///
/// If `RONAMI_PROXY` exists, but isn't correct url.
#[must_use]
pub fn client_from_env() -> reqwest::Client {
    use reqwest::Proxy;

    const RONAMI_PROXY: &str = "RONAMI_PROXY";

    let builder = default_reqwest_settings();

    match std::env::var(RONAMI_PROXY).ok() {
