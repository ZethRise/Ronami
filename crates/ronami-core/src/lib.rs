...
//! (Currently, version `10.2` is supported) with ease. The library is fully
...
//! ```
//! # async {
//! # let chat_id = ronami_core::types::ChatId(-1);
//! use ronami_core::{
//!     prelude::*,
...
//
// `dep_docsrs` is used for the same purpose, but when `ronami-core` is built as a dependency
// (see: `ronami`). We can't use `docsrs` as it breaks tokio compilation in this case.
