...
pub(crate) fn bot_commands_impl(input: DeriveInput) -> Result<TokenStream> {
    ...
    let trait_impl = quote! {
        impl ronami::utils::command::BotCommands for #type_name {
            #fn_descriptions
            #fn_parse
            #fn_commands
        }
...
fn impl_commands(infos: &[Command]) -> proc_macro2::TokenStream {
    ...
    quote! {
        fn bot_commands() -> ::std::vec::Vec<ronami::types::BotCommand> {
            use ronami::types::BotCommand;
            ::std::vec![#(#commands),*]
        }
    }
}
...
fn impl_descriptions(infos: &[Command], global: &CommandEnum) -> proc_macro2::TokenStream {
    ...
    quote! {
        fn descriptions() -> ronami::utils::command::CommandDescriptions<'static> {
            use ronami::utils::command::{CommandDescriptions, CommandDescription};
            use std::borrow::Cow;
...
fn impl_parse(
    ...
    quote! {
         fn parse(s: &str, bot_name: &str) -> ::std::result::Result<Self, ronami::utils::command::ParseError> {
              // FIXME: we should probably just call a helper function from `ronami`, instead of parsing command syntax ourselves
              use std::str::FromStr;
              use ronami::utils::command::ParseError;
