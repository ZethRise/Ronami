    ...
    let res = {
        ...
        quote! {
            (
                #(
                    {
                        let s = splitted.next().ok_or(ronami::utils::command::ParseError::TooFewArguments {
                            expected: #expected,
                            found: #found,
    ...
    let res = quote! {
        (
            |s: ::std::string::String| {
                ...
                match splitted.next() {
                    Some(d) if !s.is_empty() => ::std::result::Result::Err(ronami::utils::command::ParseError::TooManyArguments {
                        expected: #expected,
