...
//!
//! In `ronami`, update dispatching is declarative: it takes the form of a
...
//! ```no_run
//! # #[cfg(feature = "macros")] {
//! # use ronami::utils::command::BotCommands;
...
//! # use ronami::dispatching::{UpdateHandler, UpdateFilterExt, dialogue, dialogue::InMemStorage};
//! # use ronami::utils::command::BotCommands;
//! # use ronami::types::Update;
...
//!     use dptree::case;
//!
//!     let command_handler = ronami::filter_command::<Command, _>()
//!         .branch(
//!             case![State::Start]
...
//! ```no_run
//! # use ronami::Bot;
//! # use ronami::types::{Message, CallbackQuery};
//! # use ronami::dispatching::dialogue::{InMemStorage, Dialogue};
//! # enum State{}
//! #
...
//! ```
//!
//! Each parameter is supplied as a dependency by `ronami`. In particular:
...
//! # use ronami::Bot;
//! # use ronami::requests::RequesterExt;
//! # use ronami::dispatching::{Dispatcher, dialogue::InMemStorage};
//! # enum State {}
//! # fn schema() -> ronami::dispatching::UpdateHandler<Box<dyn std::error::Error + Send + Sync + 'static>> { ronami::dptree::entry() }
//! #[tokio::main]
//! async fn main() {
...
//! [dialogues]: dialogue
//! [`examples/purchase.rs`]: https://github.com/teloxide/teloxide/blob/master/crates/ronami/examples/purchase.rs
...
//! [`examples/dispatching_features.rs`]: https://github.com/teloxide/teloxide/blob/master/crates/ronami/examples/dispatching_features.rs
