...
//! ```
//!
//! [`examples/dialogue.rs`]: https://github.com/teloxide/teloxide/blob/master/crates/ronami/examples/dialogue.rs
...
const RONAMI_DIALOGUE_BEHAVIOUR: &str = "RONAMI_DIALOGUE_BEHAVIOUR";
...
///
/// If `RONAMI_DIALOGUE_BEHAVIOUR` environmental variable exists and is equal
    ...
    Output: Send + Sync + 'static,
{
    ...
    .filter_map_async(|dialogue: Dialogue<D, S>| async move {
        match dialogue.get_or_default().await {
            ...
            Err(err) => match std::env::var(RONAMI_DIALOGUE_BEHAVIOUR).as_deref() {
                Ok("default") => {
