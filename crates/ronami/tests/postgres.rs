...
async fn test_postgres_json() {
    let storage = PostgresStorage::open(
        "postgres://ronami:rewrite_it_in_rust@localhost:5432/test_postgres_json",
        1,
        ronami::dispatching::dialogue::serializer::Json,
    )
    .await
    .unwrap();
...
async fn test_postgres_bincode() {
    let storage = PostgresStorage::open(
        "postgres://ronami:rewrite_it_in_rust@localhost:5432/test_postgres_bincode",
        1,
        ronami::dispatching::dialogue::serializer::Bincode,
    )
    .await
    .unwrap();
...
async fn test_postgres_cbor() {
    let storage = PostgresStorage::open(
        "postgres://ronami:rewrite_it_in_rust@localhost:5432/test_postgres_cbor",
        1,
        ronami::dispatching::dialogue::serializer::Cbor,
    )
    .await
    .unwrap();
